...
<template>
    <div class="bg-slate-200 dark:bg-slate-800 rounded-lg p-6 m-6">
        <form @submit.prevent="form.post(route('login'), { onFinish: () => form.reset('password') })" class="flex flex-col w-96">
            <InputGroup required autofocus v-model="form.email" :errors="form.errors.email" label="Email"  />
